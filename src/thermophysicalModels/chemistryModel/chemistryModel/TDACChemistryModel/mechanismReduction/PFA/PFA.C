...
) 
{
    ...
    //If a new species is connected, add it to the list of connected species.
    //It is a connection of second generation and it will be aggregated in the
    ...
    while (!Q.empty())
    {
        ...
        if (Den!=0.0)
        {
            ...
            for (label v=0; v<NbrABInit[u]; v++)
            {
                ...
                if (rAB >= this->tolerance() && !this->activeSpecies_[otherSpec])
                {
                    Q.push(otherSpec);
