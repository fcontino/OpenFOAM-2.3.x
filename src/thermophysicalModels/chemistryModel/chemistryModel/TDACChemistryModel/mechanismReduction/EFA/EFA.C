...
)
{
    ...
    forAll(this->chemistry_.reactions(), i)
    {
        ...
        forAll(R.lhs(), Ai)//compute the element flux
        {
            ...
            forAll(R.rhs(), Bi)
            {
                ...
                //if a pair BA is initialized, add the element flux to this pair
                else
                {
    ...
    if (CFlux > VSMALL)
    {
        ...
        //reached. The sorting is done on part of the pairs because a small part
        //of these pairs are responsible for a large part of the element flux
        scalar cumFlux(0.0);
